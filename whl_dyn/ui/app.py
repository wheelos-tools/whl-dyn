...
from whl_dyn.ui.lateral import render_lateral_plan, render_lateral_collect, render_lateral_analysis
...
def render_metric_compact(label: str, value: str, rating: dict, help_text: str = None):
    ...
    html_content = f'<div style="display: flex; justify-content: space-between; align-items: center; padding: 4px 0;"><span style="font-size: 13px; color: #CCC;"{help_attr}>{safe_label}</span><div style="display: flex; align-items: center; gap: 8px;"><span style="font-size: 14px; font-weight: 600; color: {rating["color"]};">{safe_value}</span><span style="font-size: 10px; padding: 2px 6px; border-radius: 3px; background: {rating["color"]}; color: #000;">{safe_rating_label}</span></div></div>'
...
plan_default = str(PROJECT_ROOT / "plans" / "calibration_plan.yaml")
...
category = st.radio("工作类别", ["🚗 油门/刹车", "🧭 横向动力学"], horizontal=True)
...
if category == "🚗 油门/刹车":
        ...
        if view_mode == "表格":
            ...
            if plan_path.exists():
                ...
                edited_yaml = st.text_area(
                    "编辑 YAML 计划", value=plan_yaml, height=350, key=f"plan_yaml_edit_{plan_path.name}"
                )
                if st.button("💾 保存计划修改", key="btn_save_long_yaml_plan"):
                    try:
                        yaml.safe_load(edited_yaml)
                        plan_path.write_text(edited_yaml, encoding="utf-8")
    ...
    with collect_tab:
        ...
        default_plan = "plans/calibration_plan.yaml" if "plans/calibration_plan.yaml" in preset_plans else (preset_plans[0] if preset_plans else str(plan_default))
        ...
        with top1:
            plan_file_str = st.selectbox(
                ...
                key="collect_plan_select",
            )
            plan_path = resolve_path(plan_file_str)
        with top2:
            # 设置默认值，如果session state中有值就用，否则用默认值
            out_dir_text = st.text_input("输出目录", value="calibration_data_logs", key="collect_output_dir")
...
<div class="progress-list">"""
                ...
                for idx, case_name in enumerate(plan_df["case_name"].tolist()):
                    ...
                    rows_html.append(
                        f'<div class="progress-row {row_class}{selected_class}"><span>{case_name_short}</span><span style="text-align:center;">{group_count}组</span><span style="text-align:center;">{status_icon}</span><span style="text-align:right;">{total_rows}</span></div>'
                    )

                full_status_html = style_block + "".join(rows_html) + "</div>"
                st.markdown(full_status_html, unsafe_allow_html=True)
    ...
    with analysis_tab:
        ...
        with metrics_col:
            if raw_df is not None:
                ...
                range_html = f'<div style="display:flex;gap:20px;margin-bottom:8px;"><div style="font-size:11px;color:#999;">响应范围:</div><div style="font-size:13px;color:#64DD17;">油门 {max_accel:.2f} m/s²</div><div style="font-size:13px;color:#FF3D00;">刹车 {max_decel:.2f} m/s²</div></div>'
                ...
                mono_html = f'<div style="padding:6px 8px;background:#1A2E1A;border-radius:4px;border-left:2px solid {mono_color};"><div style="display:flex;justify-content:space-between;align-items:center;"><span style="font-size:11px;color:#CCC;">单调性总评</span><span style="font-size:12px;font-weight:bold;color:{mono_color};">{mono_status}</span></div><div style="margin-top:2px;font-size:10px;color:#999;">油门:{t_vio} 违反  |  刹车:{b_vio} 违反</div></div>'
                st.markdown(mono_html, unsafe_allow_html=True)
    ...
    lat_plan_tab, lat_collect_tab, lat_analysis_tab = st.tabs(
        [
            "📋 ① 生成计划",
            "🚗 ② 数据采集",
            "📊 ③ 分析",
    ...
    with lat_collect_tab:
        render_lateral_collect(RUNTIME_DIR)
    with lat_analysis_tab:
